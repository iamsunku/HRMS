...
const projectsData = [
    {
        id: '1',
        name: 'Physics Curriculum Revamp',
        description: 'Modernizing the Senior Secondary Physics curriculum with interactive simulations and AI tools.',
        status: 'IN_PROGRESS',
        priority: 'HIGH',
        progress: 65,
        teamCount: 12,
        departments: ['Academics', 'Content Development'],
        deadline: '2026-03-15'
    },
    {
        id: '2',
        name: 'LMS Mobile App Integration',
        description: 'Expanding the Learning Management System to support seamless mobile-first learning experiences.',
        status: 'PLANNING',
        priority: 'MEDIUM',
        progress: 15,
        teamCount: 8,
        departments: ['Ai Media', 'Academics'],
        deadline: '2026-05-20'
    },
    {
        id: '3',
        name: 'Faculty Upskilling 2025',
        description: 'Comprehensive training program for educators on advanced pedagogical techniques.',
        status: 'COMPLETED',
        priority: 'LOW',
        progress: 100,
        teamCount: 5,
        departments: ['HR', 'Academics'],
        deadline: '2025-12-30'
    },
    {
        id: '4',
        name: 'Academic Audit Q1',
        description: 'Evaluating departmental performance and student engagement metrics across regional centers.',
        status: 'ON_HOLD',
        priority: 'CRITICAL',
        progress: 45,
        teamCount: 15,
...
export default function ProjectsPage() {
    ...
    const [departmentHeads, setDepartmentHeads] = useState([
        { id: '1', name: 'Arjun Sharma', role: 'Physics HOD', dept: 'Academics', avatar: 'AS' },
        { id: '2', name: 'Rahul Vikram', role: 'LMS Administrator', dept: 'Ai Media', avatar: 'RV' },
        { id: '3', name: 'Priya Patel', role: 'Academic Dean', dept: 'Leadership', avatar: 'PP' },
        { id: '4', name: 'Sneha L.', role: 'Subject Matter Expert', dept: 'Academics', avatar: 'SL' },
        { id: '5', name: 'Robert Brown', role: 'Center Coordinator', dept: 'Operations', avatar: 'RB' },
    ]);
    ...
    return (
        <Shell title="Academic Programs & Courses">
            <div className="p-6 md:p-8 space-y-8 animate-fade-in max-w-[1400px] mx-auto pb-20">
                ...
                <div className="flex flex-col lg:flex-row justify-between items-end lg:items-center gap-6">
                    <div className="space-y-1">
                        <h2 className="text-2xl md:text-3xl font-bold text-slate-900 tracking-tight">Academic Initiatives</h2>
                        <div className="flex items-center gap-3 text-slate-500 font-medium text-sm">
                            <div className="flex items-center gap-1.5 pt-1">
                                <Activity size={14} className="text-violet-600" />
                                <span>12 Active Academic Sessions</span>
                            </div>
                            <div className="w-1 h-1 rounded-full bg-slate-300 mx-1" />
                            <span>Comprehensive Curriculum Development Hub</span>
                        </div>
                    </div>
                    <button
                        onClick={() => setIsCreateModalOpen(true)}
                        className="w-full sm:w-auto px-8 py-3 bg-violet-600 text-white rounded-xl text-xs font-bold uppercase tracking-widest shadow-xl shadow-violet-200 hover:bg-violet-700 transition-all flex items-center justify-center gap-3 active:scale-95 group"
                    >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                    {[
                        { label: 'Active Programs', value: projectsData.filter(p => p.status === 'IN_PROGRESS').length, icon: Activity, color: 'text-violet-600', bg: 'bg-violet-50' },
                        { label: 'Pending Assessment', value: '48', trend: 'Academic Priority', icon: Clock, color: 'text-amber-600', bg: 'bg-amber-50/50' },
                        { label: 'Faculty Assignments', value: projectsData.reduce((acc, p) => acc + p.teamCount, 0), icon: UsersIcon, color: 'text-indigo-600', bg: 'bg-indigo-50' },
                        { label: 'Completion Rate', value: '88%', icon: Target, color: 'text-emerald-600', bg: 'bg-emerald-50' },
                    ].map((stat, i) => (
                    ...
                    <div className="flex w-full md:w-auto flex-1 max-w-xl items-center gap-3">
                        <div className="relative flex-1">
                            ...
                            <input
                                ...
                                className="w-full pl-12 pr-4 py-2.5 bg-white border border-slate-200 rounded-xl text-sm font-medium outline-none focus:ring-4 focus:ring-violet-500/5 focus:border-violet-500/20 transition-all placeholder:text-slate-400"
                            />
                        </div>
                ...
                {/* Content Grid */}
                <div className={viewMode === 'grid' ? "grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 gap-8" : "space-y-4"}>
                    {projectsData.map((project) => (
                        <Link key={project.id} href={`/projects/${project.id}`} className="block h-full group">
                            <div className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-500 flex flex-col h-full relative overflow-hidden group-hover:border-violet-100">
                                <div className="absolute top-0 right-0 w-32 h-32 bg-slate-50/50 rounded-full -mr-16 -mt-16 group-hover:bg-violet-50/50 transition-colors duration-500" />

                                <div className="flex justify-between items-start mb-6 relative z-10">
                                    <div className="space-y-2 flex-1">
                                        <div className="flex items-center gap-2">
                                            <StatusDot status={project.status} />
                                            <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">{project.status.replace('_', ' ')}</span>
                                        </div>
                                        <h3 className="font-bold text-slate-900 text-lg group-hover:text-violet-600 transition-colors line-clamp-2">
                                            {project.name}
                                        </h3>
                                    </div>
                                    <div className="w-10 h-10 rounded-[1rem] bg-slate-50 text-slate-900 flex items-center justify-center shadow-inner group-hover:bg-violet-600 group-hover:text-white transition-all duration-300 shrink-0">
                                        <Briefcase size={18} />
                                    </div>
                                </div>
                                ...
                                <div className="space-y-6 mt-auto">
                                    ...
                                    <div className="flex justify-between items-center pt-5 border-t border-slate-50">
                                            ...
                                            <div className="flex -space-x-2">
                                                {[...Array(Math.min(3, project.teamCount))].map((_, i) => (
                                                    <div key={i} className="w-7 h-7 rounded-full border-2 border-white overflow-hidden shadow-sm bg-slate-100">
                                                        <img src={`https://i.pravatar.cc/100?u=${project.id}${i}`} className="w-full h-full object-cover" alt="team" />
                                                    </div>
                                                ))}
                                                {project.teamCount > 3 && (
                                                    <div className="w-7 h-7 rounded-full border-2 border-white bg-violet-600 text-white flex items-center justify-center text-[10px] font-black shadow-sm">
                                                        +{project.teamCount - 3}
                                                    </div>
                                                )}
                                            </div>
                                        ...
                                        <div className="text-right">
                                            ...
                                            <p className="text-slate-900 text-[11px] font-bold">
            ...
            {isCreateModalOpen && (
                <div className="fixed inset-0 z-[5000] flex justify-end overflow-hidden">
                    ...
                    <div className="relative bg-[#f8fafc] w-full md:max-w-3xl lg:max-w-4xl xl:max-w-5xl h-full shadow-[0_0_100px_rgba(0,0,0,0.2)] animate-in slide-in-from-right duration-700 flex flex-col">
                            ...
                            <div className="flex items-center gap-6">
                                ...
                                <div>
                                    <div className="flex items-center gap-3 mb-1">
                                        <span className="text-[10px] font-black text-violet-600 uppercase tracking-[0.2em]">Course Initiation Protocol</span>
                                        <div className="w-1 h-1 rounded-full bg-slate-300" />
                                        <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">ID: CRS-NEW-INIT</span>
                                    </div>
                                    <h2 className="text-3xl font-black text-slate-900 tracking-tight">Academic Development Workspace</h2>
                                </div>
                            </div>
                            <button
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-12 pl-14">
                                        <div className="space-y-3">
                                            <label className="text-[11px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                                                <Target size={14} className="text-violet-600" /> Project Identifier Name
                                            </label>
                                            <input type="text" className="w-full px-6 py-4 bg-white rounded-2xl border border-slate-200 focus:outline-none focus:ring-4 focus:ring-violet-500/5 focus:border-violet-500/20 transition-all text-base font-bold placeholder:text-slate-300 shadow-sm" placeholder="Ex: Global Infrastructure Overhaul" />
                                        ...
                                        <div className="space-y-3">
                                            ...
                                            <input type="date" className="w-full px-6 py-4 bg-white rounded-2xl border border-slate-200 focus:outline-none focus:ring-4 focus:ring-violet-500/5 focus:border-violet-500/20 transition-all text-sm font-bold shadow-sm" />
                                        </div>
                                        ...
                                        <div className="space-y-3 md:col-span-2">
                                            ...
                                            <textarea rows={4} className="w-full px-6 py-4 bg-white rounded-2xl border border-slate-200 focus:outline-none focus:ring-4 focus:ring-violet-500/5 focus:border-violet-500/20 transition-all text-base font-bold resize-none placeholder:text-slate-300 shadow-sm" placeholder="Define the core objectives, deliverables, and expected outcomes..." />
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="space-y-10 pt-10 border-t border-slate-100">
                                    ...
                                    <div className="pl-14 space-y-8">
                                        <div className="flex justify-between items-end mb-4">
                                            <div className="space-y-1">
                                                <p className="text-sm font-bold text-slate-900">Lead Academician</p>
                                                <p className="text-xs font-medium text-slate-500">Personnel responsible for curriculum & delivery</p>
                                            </div>
                                            ...
                                            <div className="p-8 bg-slate-50 rounded-3xl border border-slate-200 border-dashed space-y-6 animate-in slide-in-from-top-4 duration-500">
                                                <div className="grid grid-cols-2 gap-6">
                                                    <div className="space-y-2">
                                                        ...
                                                        <input type="text" value={newLeadName} onChange={(e) => setNewLeadName(e.target.value)} placeholder="Personnel Full Name" className="w-full px-5 py-3.5 bg-white rounded-xl text-sm font-bold border border-slate-200 focus:border-violet-500 outline-none transition-all shadow-sm" />
                                                    </div>
                                                    <div className="space-y-2">
                                                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Department Unit</label>
                                                        <input type="text" value={newLeadDept} onChange={(e) => setNewLeadDept(e.target.value)} placeholder="Ex: IT Operations, HR" className="w-full px-5 py-3.5 bg-white rounded-xl text-sm font-bold border border-slate-200 focus:border-violet-500 outline-none transition-all shadow-sm" />
                                                    </div>
                                                </div>
                                                <button onClick={handleAddNewLead} className="w-full py-4 bg-violet-600 text-white rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-violet-700 transition-all shadow-xl shadow-violet-100">Establish Deployment Lead</button>
                                            </div>
                                        ) : (
                                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                                {departmentHeads.map((head) => (
                                                    <div
                                                        ...
                                                        className={`p-5 rounded-2xl border-2 cursor-pointer transition-all flex items-center gap-4 ${selectedHead === head.id ? 'bg-white border-violet-600 shadow-xl shadow-violet-500/10' : 'bg-white border-slate-100 hover:border-slate-200 hover:shadow-sm'}`}
                                                    >
                                                        <div className={`w-12 h-12 rounded-xl flex items-center justify-center text-xs font-black tracking-tight ${selectedHead === head.id ? 'bg-violet-600 text-white shadow-lg shadow-violet-200' : 'bg-slate-100 text-slate-400 border border-slate-200'}`}>
                                                            {head.avatar}
                                                        </div>
                                                        <div className="min-w-0">
                                                            <p className={`text-sm font-black truncate ${selectedHead === head.id ? 'text-violet-900' : 'text-slate-900'}`}>{head.name}</p>
                        ...
                        <div className="px-10 py-8 bg-white border-t border-slate-200 flex flex-col md:flex-row gap-6 shrink-0">
                            ...
                            <div className="flex gap-4 min-w-[400px]">
                                ...
                                <button className="flex-[2] py-4 bg-slate-900 text-white rounded-2xl text-xs font-black uppercase tracking-widest shadow-2xl shadow-slate-900/20 hover:bg-slate-800 transition-all flex items-center justify-center gap-3 active:scale-[0.98]">
                                    <Save size={18} /> Launch Academic Program
                                </button>
                            </div>
                        </div>
                    </div>
